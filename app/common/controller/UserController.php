...
use app\user\service\ConfigService;
...
class UserController extends BaseController
{
     ...
     * @return array
     */
    protected function buildTableParames($excludeFields = [],$user=false)
    {
    ...
    private function checkAuth(){
        ...
        if (!in_array($currentController, $userConfig['no_login_controller']) &&
            ...
            empty($userId) && $this->error('请先登录后台', [], __url('user/login/index'));

            // 判断是否登录过期
            if ($expireTime !== true && time() > $expireTime) {
                session('user', null);
                $this->error('登录已过期，请重新登录', [], __url('user/login/index'));
            }
        }
